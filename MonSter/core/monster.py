...
from MonSter.core.update import BasicMultiUpdateBlock, BasicMultiUpdateBlock_mix2
from MonSter.core.geometry import Combined_Geo_Encoding_Volume
from MonSter.core.submodule import *
from MonSter.core.refinement import REMP
from MonSter.core.warp import disp_warp
...
from MonSter.depth_anything_v2.dpt import DepthAnythingV2, DepthAnythingV2_decoder
...
class Monster(nn.Module):
    def __init__(self, config):
        ...
        dim_list_ = mono_model_configs[self.config["encoder"]]['features']
        dim_list = []
        dim_list.append(dim_list_)
        self.update_block = BasicMultiUpdateBlock(self.config, hidden_dims=config['hidden_dims'])

        self.context_zqr_convs = nn.ModuleList([nn.Conv2d(context_dims[i], config['hidden_dims'][i]*3, 3, padding=3//2) for i in range(self.config["n_gru_layers"])])

        self.feat_transfer = Feat_transfer(dim_list)
        self.feat_transfer_cnet = Feat_transfer_cnet(dim_list, output_dim=config['hidden_dims'][0])
        ...
        depth_anything = DepthAnythingV2(**mono_model_configs[config["encoder"]])
        depth_anything_decoder = DepthAnythingV2_decoder(**mono_model_configs[config["encoder"]])
        state_dict_dpt = torch.load(f'{config["pretrained_dir"]}/depth_anything_v2_{config["encoder"]}.pth', map_location='cpu')
        # state_dict_dpt = torch.load(f'./pretrained/depth_anything_v2_{config["encoder"]}.pth', map_location='cpu')
        ...
        self.update_block_mix_stereo = BasicMultiUpdateBlock_mix2(self.config, hidden_dims=config["hidden_dims"])
        self.update_block_mix_mono = BasicMultiUpdateBlock_mix2(self.config, hidden_dims=config["hidden_dims"])
    ...
    def infer_mono(self, image1, image2):
        ...
        features_left_encoder = self.mono_encoder.get_intermediate_layers(resize_image1, self.intermediate_layer_idx[self.config["encoder"]], return_class_token=True)
        features_right_encoder = self.mono_encoder.get_intermediate_layers(resize_image2, self.intermediate_layer_idx[self.config["encoder"]], return_class_token=True)
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, test_mode=False):
        ...
        gwc_volume = build_gwc_volume(match_left, match_right, self.config["max_disp"]//4, 8)
        ...
        init_disp = disparity_regression(prob, self.config["max_disp"]//4)
        ...
        geo_fn = geo_block(match_left.float(), match_right.float(), geo_encoding_volume.float(), radius=self.config["corr_radius"], num_levels=self.config["corr_levels"])
        ...
        for itr in range(iters):
            ...
            if itr <= int(iters-8):
                net_list, mask_feat_4, delta_disp = self.update_block(net_list, inp_list, geo_feat, disp, iter16=self.config["n_gru_layers"]==3, iter08=self.config["n_gru_layers"]>=2)
            else:
                net_list, mask_feat_4, delta_disp = self.update_block_mix_stereo(net_list, inp_list, flaw_stereo, disp, geo_feat, flaw_mono, disp_mono_4x, geo_feat_mono, iter16=self.config["n_gru_layers"]==3, iter08=self.config["n_gru_layers"]>=2)
                net_list_mono, mask_feat_4_mono, delta_disp_mono = self.update_block_mix_mono(net_list_mono, inp_list, flaw_mono, disp_mono_4x, geo_feat_mono, flaw_stereo, disp, geo_feat, iter16=self.config["n_gru_layers"]==3, iter08=self.config["n_gru_layers"]>=2)

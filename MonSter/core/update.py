...
class BasicMotionEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        cor_planes = config["corr_levels"] * (2*config["corr_radius"] + 1) * (8+1)
...
class BasicMultiUpdateBlock(nn.Module):
    def __init__(self, config, hidden_dims=[]):
        ...
        self.gru04 = ConvGRU(hidden_dims[2], encoder_output_dim + hidden_dims[1] * (config["n_gru_layers"] > 1))
        self.gru08 = ConvGRU(hidden_dims[1], hidden_dims[0] * (config["n_gru_layers"] == 3) + hidden_dims[2])
        self.gru16 = ConvGRU(hidden_dims[0], hidden_dims[1])
        self.disp_head = DispHead(hidden_dims[2], hidden_dim=256, output_dim=1)
        factor = 2**self.config["n_downsample"]
...
class BasicMultiUpdateBlock_mix2(nn.Module):
    def __init__(self, config, hidden_dims=[]):
        super().__init__()
        self.config = config
        self.encoder = BasicMotionEncoder_mix2(config)
        encoder_output_dim = 128
...
class BasicMotionEncoder_mix2(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        cor_planes = 96 + config["corr_levels"] * (2*config["corr_radius"] + 1) * (8+1)
        self.convc1 = nn.Conv2d(cor_planes, 64, 1, padding=0)
        self.convc2 = nn.Conv2d(64, 64, 3, padding=1)
